# Session: 3b126910-493a-4dca-86ff-346cf07132d0
...
Sapling was designed for Meta's **massive single monorepo** approach—they explicitly avoided splitting into submodules. Per [their docs](https://sapling-scm.com/docs/introduction/):
...
This actually fits your model—independent versioned repos that compose into a hierarchy.
...
|----------|---------------------|----------|-------|
| **Google `repo`** | ✅ Yes | High | Best for independent versioned subrepos |
| **Git Submodules** | ⚠️ Painful | High | Works, but recursive nesting hurts |
| **Jujutsu** | ❌ No | Low (for submodules) | No submodule support yet |
| **Sapling** | ❌ No | High | Designed for single monorepo |
| **git-subrepo** | ❌ No | Medium | Too buggy for serious use |
...
|------|---------------------|-----|
| **Git Submodules** | ⚠️ Painful but works | Most mature, but nested = pain multiplied |
| **Jujutsu (jj)** | ❌ No | Zero submodule support, manual workarounds only |
| **Google repo** | ❌ Overkill | Designed for Android-scale, no personal use evidence |
| **git-subrepo** | ❌ Buggy | Active issues with basic pull/push workflows |
| **Sapling** | ❌ Wrong model | Designed for single monorepo, not multi-repo |
...
```
life-repo/
├── dotfiles/           # git subtree or flat
├── automations/        # flat
├── projects/
│   ├── project-a/      # npm publish / cargo publish as needed
│   └── project-b/      # independent releases via package manager
└── notes/              # flat
```
...
```
life-repo/
├── dotfiles/                    # flat - no need for separate repo
├── automations/                 # flat
├── notes/                       # flat
├── forks/
│   └── super-productivity/      # SUBMODULE - your fork, tracks upstream
├── projects/
│   ├── project-a/               # flat - use subtree push for public release
│   └── project-b/               # flat - use subtree push for public release
```
...
1. **Reproducible environment across multiple systems** ← submodules' strength
2. **Track forks with your modifications** ← submodules
3. **Work across all code in one place** ← monorepo
4. **Publish clean repos for end users** ← submodules give this for free
...
```
life-repo/
├── dotfiles/                    # submodule → github.com/you/dotfiles
├── automations/                 # submodule → github.com/you/automations  
├── forks/
│   └── super-productivity/      # submodule → github.com/you/super-productivity-fork
├── projects/
│   ├── project-a/               # submodule → github.com/you/project-a
│   └── project-b/               # submodule → github.com/you/project-b
└── notes/                       # submodule (private) or flat
```
...
```
life-repo/
└── desktop-config/           # submodule (level 1)
    └── super-productivity/   # submodule (level 2)
        └── some-dependency/  # submodule (level 3)
```
...
- ❌ No independent version tracking
- ❌ Can't push changes back to the fork
- ❌ Bloats your life-repo with duplicate files
- ❌ Loses all commit history of the fork
...
Result:
- ❌ Broken pseudo-submodule
- ❌ Other machines can't clone it properly
- ❌ Confusing state
...
This:
- ✅ Clones the repo
- ✅ Creates `.gitmodules` entry with URL
- ✅ Creates gitlink (pointer to specific commit)
- ✅ Other machines can init/update properly
- ✅ Independent version tracking preserved
...
```bash
# ❌ NEVER use these inside life-repo or any of its submodules
...
# ✅ For repos that already have submodules inside them
